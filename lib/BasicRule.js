...
class BasicRule {
    ...
    loadWithStream(streamSample) {
        if (!streamSample || streamSample.constructor.name !== 'ReadStream') {
            throw new Error('Instance must be ReadStream');
        }
        this.htmlString = '';
        streamSample.on('data', chunk => this.htmlString += chunk);
        streamSample.on('end', () => this.readStreamEvent.emit('readEnd', this));
        streamSample.on('error', err => this.readStreamEvent.emit('error', err));
    }
    /**
     * input a file for searching
     * @param {string}} filePath the file destination
     * @returns void
     */
    loadWithFile(filePath) {
        ...
        this.loadWithStream(fs.createReadStream(filePath, { encoding: 'utf-8' }));
    }
    /**
